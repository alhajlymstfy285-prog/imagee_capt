...
def scaled_dot_product_two_loop_single(
    ...
    Similarities = Similarities / torch.sqrt(torch.tensor(K,dtype=Similarities.dtype))
    attention_weigth = torch.softmax(Similarities,dim=-1)
    out=attention_weigth @ value
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    Similarities = torch.bmm(query , key.transpose(1,2) )
    Similarities = Similarities / torch.sqrt(torch.tensor(M,dtype=Similarities.dtype))
    attention_weigth = torch.softmax(Similarities,dim=-1)
    out=torch.bmm(attention_weigth , value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        if len(mask.shape) != len(Similarities.shape):
            mask = mask.reshape(1,mask.shape[0],mask.shape[1])
            mask = mask.repeat(Similarities.shape[0],1,1)
        Similarities = Similarities.masked_fill(mask,float('-inf'))
    # Replace "pass" statement with your code
    weights_softmax = torch.softmax(Similarities,dim=-1)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.Multihead = nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for x in range(num_heads)])
        self.linear = nn.Linear(num_heads * dim_out, dim_in)
        # Initialize linear layer weights
        c = math.sqrt(6 / (self.linear.in_features + self.linear.out_features))
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        outputs =[]
        for model in self.Multihead:
            out = model(query,key,value,mask)
            outputs.append(out)
        combined = torch.cat(outputs,dim=-1)
        y=self.linear(combined)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        self.gama = nn.Parameter(torch.ones(emb_dim))
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.LayerNormalization1=LayerNormalization(emb_dim)
        self.LayerNormalization2=LayerNormalization(emb_dim)

        self.FeedForwardBlock=FeedForwardBlock(emb_dim,feedforward_dim)
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1 = self.MultiHeadAttention(x,x,x) # N , K , M
        out1 = self.LayerNormalization1(out1+x)
        out2 = self.dropout(out1)

        out3 = self.FeedForwardBlock(out2)
        out3=self.LayerNormalization2((out2+out3))
        y = self.dropout(out3)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    pos = torch.arange(K, dtype=torch.float32) / K  # شكلها (K,)
    # نكررها M مرة على البُعد الأخير
    y = pos.unsqueeze(1).repeat(1, M)  # شكلها (K, M)
    return y.unsqueeze(0)  # شكلها النهائي (1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    y = torch.zeros((K, M))
    y[:, 0::2] = torch.sin(position)  # All even columns: sin(position)
    y[:, 1::2] = torch.cos(position)  # All odd columns: cos(position)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        enc_0ut = self.encoder(q_emb_inp)
        mask = get_subsequent_mask(a_emb_inp)
        dec_out = self.decoder(a_emb_inp, enc_0ut, mask)
