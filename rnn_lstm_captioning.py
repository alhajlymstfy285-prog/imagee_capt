...
def rnn_step_backward(dnext_h, cache):
    ...
    # dx = ((1-out**2) * dnext_h) @ Wx.T
    # dprev_h = dnext_h * (1-out**2) @ Wh.T
    # dWx = x.T @ (dnext_h * (1-out**2) @ Wh.T)
    dtanh = (1 - out**2) * dnext_h
    # dWh = prev_h.T @ dtanh
    # db = (1-out**2).sum(dim=0)
    dx = dtanh @ Wx.T
    dprev_h = dtanh @ Wh.T
    dWx = x.T @ dtanh
    dWh = prev_h.T @ dtanh
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    H = Wh.shape[0]
    # h = torch.zeros(N, T, H, **to_double) # Removed in-place modification
    h_list = [] # Store hidden states in a list
    cache = []
    prev_h = h0 # Start with initial hidden state

    for t in range(T):
        next_h, cache_t = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b)
        h_list.append(next_h) # Append to list
        cache.append(cache_t)
        prev_h = next_h # Update previous hidden state


    h = torch.stack(h_list, dim=1) # Stack list into a tensor
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    # Replace "pass" statement with your code
    N, T, H = dh.shape
    x, prev_h, Wx, Wh, b = cache[0]  # Get shapes from the first timestep's cache
    D = x.shape[1]
    ...
    dprev_h = torch.zeros(N, H, dtype=dh.dtype, device=dh.device)  # Initialize for carrying gradient
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.word_embedd = WordEmbedding(vocab_size, wordvec_dim)
        # Use actual encoder output channels instead of input_dim
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        hn = torch.stack(h_list, dim=1) # Stack list into a tensor
        ##########################################################################
        #                             END OF YOUR CODE                           #
        ##########################################################################
        # return h
...
def dot_product_attention(prev_h, A):
    ...
    # Reshape A from (N, H, 4, 4) to (N, H, 16) - flatten spatial dimensions
    A_flat = A.reshape(N, H, D_a * D_a)  # (N, H, 16)
    
    # Compute attention scores: dot product between prev_h and each spatial location
    # prev_h: (N, H) -> (N, 1, H)
    # A_flat: (N, H, 16)
    # scores: (N, 1, H) @ (N, H, 16) = (N, 1, 16)
    prev_h_expanded = prev_h.unsqueeze(1)  # (N, 1, H)
    scores = (prev_h_expanded @ A_flat).squeeze(1)  # (N, 16)
    
    # Apply scaling by sqrt(H) for scaled dot-product attention
    scores = scores / torch.sqrt(torch.tensor(H, dtype=scores.dtype))
    
    # Apply softmax to get attention weights
    attn_weights_flat = torch.softmax(scores, dim=1)  # (N, 16)
    
    # Reshape attention weights back to (N, 4, 4)
    attn_weights = attn_weights_flat.reshape(N, D_a, D_a)
    
    # Compute weighted sum: attention output
    # A_flat: (N, H, 16), attn_weights_flat: (N, 16)
    # attn_weights_flat.unsqueeze(2): (N, 16, 1)
    # A_flat @ attn_weights_flat.unsqueeze(2): (N, H, 16) @ (N, 16, 1) = (N, H, 1)
    attn = (A_flat @ attn_weights_flat.unsqueeze(2)).squeeze(2)  # (N, H)
...
class AttentionLSTM(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, attn_dim: int = 1280):
        ...
        Initialize a LSTM. Model parameters to initialize:
            ...
            b: Biases, of shape (4H,)
            attn_projection: Projection layer from attn_dim to hidden_dim
        """
        super().__init__()
        ...
        # Projection layer to project CNN features from attn_dim to hidden_dim
        # Using Conv2d with 1x1 kernel to project spatial features
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        Args:
            x: Input data for the entire timeseries, of shape (N, T, D)
            A: The CNN feature activation, of shape (N, attn_dim, 4, 4)

        Returns:
            hn: The hidden state output
        """
        
        # Project CNN features from (N, attn_dim, 4, 4) to (N, H, 4, 4)
        A_proj = self.attn_projection(A)  # (N, H, 4, 4)
        ...
        h0 = A_proj.mean(dim=(2, 3))  # Initial hidden state, of shape (N, H)
